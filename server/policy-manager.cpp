...
// TODO: This must be removed when ClientManager is used
PolicyStorage policyStorage("/opt/etc/dpm/policy/PolicyManifest.xml");
}
...
void PolicyManager::initialize()
{
	...
	for (const auto& policy : defaultPolicies) {
		...
		file.create(640);
		try {
			// TODO: This must be replaced to use ClientManager
			state = policyStorage.getPolicy(policy.first).getContent();
