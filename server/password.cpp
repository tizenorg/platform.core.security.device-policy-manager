...
int createNotificationLaunch(void)
{
    ...
    if (passwdNoti != NULL) {
        ...
        if (notification_set_text(passwdNoti, NOTIFICATION_TEXT_TYPE_TITLE, "Change password", NULL, NOTIFICATION_VARIABLE_TYPE_NONE) != NOTIFICATION_ERROR_NONE) {
            notification_free(passwdNoti);
            return -1;
        }
        if (notification_set_text(passwdNoti, NOTIFICATION_TEXT_TYPE_CONTENT, "Tap here to change password", NULL, NOTIFICATION_VARIABLE_TYPE_NONE) != NOTIFICATION_ERROR_NONE) {
            notification_free(passwdNoti);
            return -1;
        }
