...
static notification_h passwdNoti;
...
int createNotificationLaunch(void)
{
    ...
    passwdNoti = notification_create(NOTIFICATION_TYPE_ONGOING);
    if (passwdNoti != NULL) {
    ...
    __context(ctxt)
{
    ...
    passwdNoti = NULL;
    if (GetPasswordPolicy(__context, "password-status") == PasswordPolicy::DPM_PASSWORD_STATUS_CHANGE_REQUIRED) {
        std::cerr << "Display Noti" << std::endl;
        createNotificationLaunch();
    }
}
