    ...
    : context(ctx)
{
    ...
    context.registerParametricMethod(this, (std::vector<std::string>)(ZoneAppManager::getIdList)(std::string));
...
ZoneAppManager::AppInfo ZoneAppManager::get(const std::string& name, const std::string& appid)
{
    ...
    try {
        ...
        application.type = appinfo.getType();
...
std::vector<std::string> ZoneAppManager::getIdList(const std::string& name)
{
    try {
        runtime::User user(name);
