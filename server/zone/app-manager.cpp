    ...
    : context(ctx)
{
    ...
    context.registerParametricMethod(this, (std::vector<ZoneAppManager::AppInfo>)(ZoneAppManager::getList)(std::string));
...
ZoneAppManager::AppInfo ZoneAppManager::get(const std::string& name, const std::string& appid)
{
    ...
    try {
        ...
        application.package = appinfo.getPackageId();

        application.icon = appinfo.getIcon();
        application.label = appinfo.getLabel();

        application.componentType = appinfo.getComponentType();
...
std::vector<ZoneAppManager::AppInfo> ZoneAppManager::getList(const std::string& name)
{
    ...
    try {
        ...
        for (const auto& appid : list) {
            result.push_back(get(name, appid));
        }
    } catch (runtime::Exception& e) {
