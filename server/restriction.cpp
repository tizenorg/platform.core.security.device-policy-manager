...
bool IsPolicyAllowed(PolicyControlContext& context, const std::string& name)
{
	return context.getPolicy(name) == "allowed" ? true : false;
}

bool IsPolicyEnabled(PolicyControlContext& context, const std::string& name)
{
	return context.getPolicy(name) == "enabled" ? true : false;
}

void SetPolicyAllowed(PolicyControlContext& context, const std::string& name, bool allow)
{
	context.updatePolicy(name, allow ? "allowed" : "disallowed");
}

void setPolicyEnabled(PolicyControlContext& context, const std::string& name, bool enable)
{
...
{
	SetPolicyAllowed(context, "camera", state);
	return 0;
}
...
{
	SetPolicyAllowed(context, "microphone", state);
	return 0;
}
