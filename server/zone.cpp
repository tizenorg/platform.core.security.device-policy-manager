...
static const char *defaultGroups[] = {
    "audio", "video", "display", "log", NULL
};

static const char *defaultAppDir[] = {
...
static void waitForSetupWizard(const std::string& watch)
{
    int fd = ::inotify_init();
    if (fd < 0) {
...
static std::string prepareDirectories(const runtime::User& user)
{
...
static void maskUserServices(const std::string& pivot, const runtime::User& user)
{
