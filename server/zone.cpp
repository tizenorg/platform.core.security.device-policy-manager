    ...
    : context(ctx)
{
    ...
    manager.createNotification("Zone::signal");
}
...
int Zone::createZone(const std::string& name, const std::string& setupWizAppid)
{
    ...
    auto create = [&manager, name, setupWizAppid] {
        ...
        try {
            ...
            ERROR(e.what());
            manager.notify("Zone::signal", name, (int)Signal::CreateSucceed);
        }

        manager.notify("Zone::signal", name, (int)Signal::CreateFailed);
    };

    manager.addAsyncWork(create);
...
int Zone::removeZone(const std::string& name)
{
    ...
    auto remove = [&manager, name] {
        ...
        try {
            ...
            ERROR(e.what());
            manager.notify("Zone::signal", name, (int)Signal::RemoveFailed);
            return;
        }
        manager.notify("Zone::signal", name, (int)Signal::RemoveSucceed);
    };

    manager.addAsyncWork(remove);
