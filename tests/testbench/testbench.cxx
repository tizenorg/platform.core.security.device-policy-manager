...
{
    if (failureCount > 0) {
        std::cout << "\nThere were " << failureCount << " failures" << std::endl;
    } else {
...
void TestSuite::run()
{
    ...
    TestCaseRegistry::iterator iter = registry.begin();
...
bool TestSuite::check(long expected, long actual, const std::string& file, long line)
{
    ...
    Testbench::report(testName, Source(file, line, stream.str()));
...
void Testbench::run()
{
    ...
    TestSuiteRegistry::iterator iter = testSuites.begin();
