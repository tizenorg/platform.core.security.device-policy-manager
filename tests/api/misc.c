...
static int misc_camera(struct testcase *tc)
{
	...
	if (dpm_allow_camera(handle, true) != 0) {
		dpm_destroy_client(handle);
		return TEST_FAILED;
	}

	enable = dpm_is_camera_allowed(handle);
	if (enable == true) {
		dpm_destroy_client(handle);
...
static int misc_microphone(struct testcase *tc)
{
	...
	if (dpm_allow_microphone(handle, true) != 0) {
		dpm_destroy_client(handle);
		return TEST_FAILED;
	}

	enable = dpm_is_microphone_allowed(handle);
	if (enable == true) {
		dpm_destroy_client(handle);
		return TEST_SUCCESSED;
	}
...
static int misc_location(struct testcase *tc)
{
	...
	if (dpm_allow_location(handle, true) != 0) {
		dpm_destroy_client(handle);
		return TEST_FAILED;
	}

	enable = dpm_is_location_allowed(handle);
	if (enable == true) {
		dpm_destroy_client(handle);
		return TEST_SUCCESSED;
	}
...
static int misc_sd_card(struct testcase *tc)
{
	...
	if (dpm_allow_sd_card(handle, true) != 0) {
		dpm_destroy_client(handle);
		return TEST_FAILED;
	}

	enable = dpm_is_sd_card_allowed(handle);
	if (enable == true) {
		dpm_destroy_client(handle);
		return TEST_SUCCESSED;
	}
