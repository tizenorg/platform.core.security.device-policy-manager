...
static int admin_register_client(struct testcase* tc)
{
	...
	ret = TEST_SUCCESSED;
	if (dpm_admin_register_client(handle, "org.tizen.dpm-toolkit", (uid_t)uid) != DPM_ERROR_NONE) {
		ret = TEST_FAILED;
	}
...
static int admin_deregister_client(struct testcase* tc)
{
	...
	ret = TEST_SUCCESSED;
	if (dpm_admin_deregister_client(handle, "org.tizen.dpm-toolkit", (uid_t)uid) != DPM_ERROR_NONE) {
		ret = TEST_FAILED;
	}
