...
static int test_dpm_settings_changes(struct testcase* tc)
{
	...
	if (dpm_restriction_set_settings_change_state(handle, true) != 0) {
		dpm_context_destroy(handle);
		return TEST_FAILED;
	}

	enable = dpm_restriction_get_settings_change_state(handle);
	if (enable == true) {
		dpm_context_destroy(handle);
		return TEST_SUCCESSED;
	}
...
static int test_dpm_usb_debugging(struct testcase* tc)
{
	...
	if (dpm_restriction_set_usb_debugging_state(handle, true) != 0) {
		dpm_context_destroy(handle);
		return TEST_FAILED;
	}

	enable = dpm_restriction_get_usb_debugging_state(handle);
	if (enable == true) {
		dpm_context_destroy(handle);
		return TEST_SUCCESSED;
	}
