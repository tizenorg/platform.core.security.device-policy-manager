...
static int test_dpm_usb_mass_storage(struct testcase* tc)
{
	...
	if (dpm_storage_set_usb_mass_storage_state(policy, true) != 0) {
		dpm_context_destroy(context);
		return TEST_FAILED;
	}

	enable = dpm_storage_get_usb_mass_storage_state(policy);
	if (enable == true) {
		dpm_context_destroy(context);
		return TEST_SUCCESSED;
	}

	dpm_context_release_storage_policy(policy);
	dpm_context_destroy(context);
	return TEST_FAILED;
}
...
static int test_dpm_wipe(struct testcase* tc)
{
	...
	if (dpm_device_wipe_data(policy) != 0) {
		dpm_context_destroy(policy);
		return TEST_FAILED;
	}
