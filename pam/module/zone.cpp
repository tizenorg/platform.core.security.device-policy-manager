                    ...
                    int argc, const char* argv[])
{
    ...
    int unshare_flags = 0;
    std::string dest, src, type, opts, create, optional;
    std::string user;
    pid_t pid;
    ...
    if (!(pidfile >> pid).good()) {
        ...
        if (unshare_flags & CLONE_NEWUTS) {
        ...
        } else if (unshare_flags & CLONE_NEWNS) {
            ...
            nodes = bundle->evaluate("//bundle-manifest/namespace/filesystem");
            for (xml::Node::NodeList::iterator it = nodes.begin();
                    it != nodes.end(); it++) {
        ...
        } else if (unshare_flags & CLONE_NEWNET) {
        } else if (unshare_flags & CLONE_NEWIPC) {
        }
