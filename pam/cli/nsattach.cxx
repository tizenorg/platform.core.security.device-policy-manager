...
static inline int attach_program(Shadow::User& user, char** args)
{
    Runtime::PAMApp pam_app("nsattach", user.getName());
    ...
    try {
        pam_app.accountManagement(0);
    } catch (Runtime::Exception& e) {
        if (e.getCode() == PAM_NEW_AUTHTOK_REQD) {
            try {
                pam_app.changeAuthenticationToken(PAM_CHANGE_EXPIRED_AUTHTOK);
            } catch (Runtime::Exception& e) {
    ...
    case 0:
        ...
        if (args == NULL) {
            args = new char*[2];
            args[0] = strdup(user.getShell().c_str());
            args[1] = NULL;
        }
