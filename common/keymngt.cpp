...
int Keymngt::validateKeyring(void)
{
    ...
    if ((rcLong = runtime::Keyctl::keyctlLink(KEY_SPEC_USER_KEYRING, KEY_SPEC_SESSION_KEYRING))) {
        rc = errno;
        goto out;
    }
out:
...
int Keymngt::addUserAuthTokenToKeyring(ecryptfs_payload* authTok)
{
    ...
    ret = runtime::Keyctl::keyctlSearch(KEY_SPEC_USER_KEYRING, "user", (const char*)authTok->token.password.signature, 0);
    if (ret != -1) {
        return 0;
    } else if (ret == -1 || errno != ENOKEY) {
        ret = runtime::Keyctl::addKey("user",
