...
void print_rule(void)
{
    ...
    printf("bluetooth\n");
    printf("Usage: dpm-cml-toolkit -s [policy-name]\n");
    printf("Ex: dpm-cml-toolkit -s password\n");
    printf("--------------------------------------------------------------\n");
    printf("[Restrictioin Polocy]\n");
    printf("camera, microphone, location, clipboard, settings, usb_debugging\n");
    printf("Usage: dpm-cml-toolkit -s [policy-name] -v [value: 0 or 1]\n");
    printf("Ex: dpm-cml-toolkit -s camera -v 1\n");
    printf("--------------------------------------------------------------\n");
}
    ...
    while ((dpm_opt = getopt(argc, argv, "s:g:v:h")) != -1) {
        ...
        case '?':
            if ((optopt == 's') | (optopt == 'v'))
                ...
                printf("Usage: dpm-cml-toolkit -g [policy-name]\n");
            else
                printf("Usage: dpm-cml-toolkit -s [policy-name] -v [value: 0 or 1] or dpm-cml-toolkit -g [policy-name]\n");
            return 0;
            break;
        }
    }
