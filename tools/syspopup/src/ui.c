...
void _create_syspopup(const char *id, char *style, const char *status, const char *user_data)
{
	...
	dlog_print(DLOG_DEBUG, LOG_TAG, "STYLE = %s, content = %s, title = %s", style, header, body);
	...
	if (!strcmp(info->style, "default")) {
		...
		//	evas_object_smart_callback_add(left_btn, "clicked", info->left_btn_cb, NULL);
		}
		...
		//	evas_object_smart_callback_add(right_btn, "clicked", info->right_btn_cb, NULL);
		}
	} else {
		elm_object_text_set(popup, body);
