...
void _create_syspopup(const char *id, char *style, const char *status, char **user_data)
{
	...
	if (!strcmp(info->style, "default")) {
		...
		if (info->left_btn != NULL) {
			...
			evas_object_smart_callback_add(left_btn, "clicked", info->left_btn_cb, &ud);
			eext_object_event_callback_add(popup, EEXT_CALLBACK_BACK, info->left_btn_cb, (void *)&ud);
		...
		if (info->right_btn != NULL) {
			...
			evas_object_smart_callback_add(right_btn, "clicked", info->right_btn_cb, &ud);
		}
	} else {
		elm_object_text_set(popup, body);
