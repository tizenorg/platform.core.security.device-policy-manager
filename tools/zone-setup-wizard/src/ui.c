...
static Eina_Bool __progressbar_timer_cb(void *data)
{
	...
	value = elm_progressbar_value_get(ud.progressbar);
	snprintf(percent, 5, "%.f %%", value*100);
	elm_object_part_text_set(ud.progressbar, "elm.text.top.right", percent);
	...
	value = value + 0.02;
	elm_progressbar_value_set(ud.progressbar, value);
...
void _create_setup_view(appdata_s *ad)
{
	...
	ud.progressbar = _create_progressbar(setup_layout, "default");
	elm_object_part_content_set(setup_layout, "progressbar", ud.progressbar);
