...
static void create_base_window(void *data)
{
	...
	if (res_path != NULL) snprintf(edj_path, 1024, "%s%s", res_path, "org.tizen.zone-setup-wizard.edj");
...
void create_zone_done(zone_state_e event, const char *name, void *info, void *user_data)
{
	...
	if (event == DPM_ZONE_DEFINED) ad->create_done = true;
	return ;
}
...
int main(int argc, char *argv[])
{
	...
	if (ret != APP_ERROR_NONE) dlog_print(DLOG_ERROR, LOG_TAG, "ui_app_main is failed. err = %d", ret);
