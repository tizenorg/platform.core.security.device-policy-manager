...
#define __DPM_DOC_H__
  ...
  * The device policy clients must create the handle by using dpm_create_context()
  * before attempting to use almost any of the device policy APIs, and the handle should be
  * freed by using dpm_destroy_context() when interaction with the device policy manager
