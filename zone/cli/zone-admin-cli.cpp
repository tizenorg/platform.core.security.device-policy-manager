...
#include <zone/package-manager.h>
#include <zone/app-manager.h>
...
int showPkgInfo(const std::string& name)
{
    ...
    zone_package_manager_foreach_package_info(pkgMgr, name.c_str(), PackgeListCallback, &num);
...
int showAppInfo(const std::string& name)
{
    ...
    zone_app_manager_h appMgr;

    zone_app_manager_create(&appMgr);
    zone_app_manager_foreach_app_info(appMgr, name.c_str(), ApplicationListCallback, &num);
    std::cout << num << " applications are found" << std::endl;
    zone_app_manager_destroy(appMgr);
