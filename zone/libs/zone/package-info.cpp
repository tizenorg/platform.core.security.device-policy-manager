...
static package_info_h make_package_info_h(const ZonePackageManager::PkgInfo& info)
{
    ...
    package_x* package = (package_x*)::malloc(sizeof(package_x));
    pkgmgr_pkginfo_x* pkginfo = (pkgmgr_pkginfo_x*)::malloc(sizeof(pkgmgr_pkginfo_x));
    package_info_s* packageinfo = (package_info_s*)::malloc(sizeof(struct package_info_s));

    ::memset(packageinfo, 0, sizeof(package_info_s));
    packageinfo->package = ::strdup(info.id.c_str());
    packageinfo->pkgmgr_pkginfo = pkginfo;

    ::memset(pkginfo, 0, sizeof(pkgmgr_pkginfo_x));
    pkginfo->uid = zone_uid;
    pkginfo->locale = ::strdup(info.locale.c_str());
    pkginfo->pkg_info = package;

    ::memset(package, 0, sizeof(package_x));
    ...
    package->icon = ::g_list_append(NULL, ::strdup(info.icon.c_str()));
    package->label = ::g_list_append(NULL, ::strdup(info.label.c_str()));
    for (const std::string &application : info.applicationList) {
        package->application = ::g_list_append(package->application,
                                               ::strdup(application.c_str()));
    }
...
int zone_package_manager_foreach_package_info(zone_package_manager_h handle, const char* name, package_manager_package_info_cb callback, void *user_data)
{
    ...
    for (const auto& info : list) {
        package_info_h info_h = make_package_info_h(info);
