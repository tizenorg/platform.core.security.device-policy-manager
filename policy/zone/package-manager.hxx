...
public:
    ...
    //package information
    struct PkgInfo {
        std::string zone, id, locale;
        std::string type, icon, label, description;
        struct {
            std::string name, email, href;
            REFLECTABLE(name, email, href);
        } author;
        std::string version, apiVersion, mainAppId;
        bool isSystem, isRemovable, isPreload;
        std::vector<std::string> applicationList;

        REFLECTABLE(zone, id, locale,
                    type, icon, label, description, author,
                    version, apiVersion, mainAppId,
                    isSystem, isRemovable, isPreload,
                    applicationList);
    };

    PkgInfo get(const std::string& name, const std::string& pkgid);
    std::vector<std::string> getIdList(const std::string& name);
