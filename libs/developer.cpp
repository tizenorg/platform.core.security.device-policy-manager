...
{
	try {
		return context->methodCall<int>("DeveloperPolicy::setUsbDebuggingRestriction", enable);
...
{
	try {
		return context->methodCall<bool>("DeveloperPolicy::isUsbDebuggingRestricted");
	} catch (runtime::Exception& e) {
		return -1;
	}
}
