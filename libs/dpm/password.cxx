...
static void __dpm_free_password(void *data)
{
...
dpm_data_type *dpm_get_password_policy(dpm_client_h handle, const char *p_username)
{
    ...
    dpm_data->mem_header.free_func = __dpm_free_password;

    dpmPasswdPolicy = (dpm_password_policy_type *)g_malloc0(sizeof(dpm_password_policy_type));
    dpmPasswdPolicy->quality_type = password.getPasswordQuality(username);
    dpmPasswdPolicy->min_length = password.getPasswordMinimumLength(username);
    dpmPasswdPolicy->min_complex_char_number = password.getMinPasswordComplexChars(username);
    dpmPasswdPolicy->max_attempts = password.getMaximumFailedPasswordForWipe(username);
    dpmPasswdPolicy->valid_period = password.getPasswordExpires(username);
    dpmPasswdPolicy->history_size = password.getPasswordHistory(username);
    dpmPasswdPolicy->max_inactivity_time = password.getMaxInactivityTimeDeviceLock(username);
    dpmPasswdPolicy->max_char_occurrences = password.getMaximumCharacterOccurrences(username);
    dpmPasswdPolicy->max_num_seqLength = password.getMaximumNumericSequenceLength(username);
    ...
    dpmPasswdPolicy->forbidden_string_list = p_list;
