...
int dpm_zone_create(dpm_zone_policy_h handle, const char* name, const char* pkgname)
{
    ...
    return zone.createZonePolicy(name, pkgname);
}
...
int dpm_zone_destroy(dpm_zone_policy_h handle, const char* name)
{
    ...
    ZonePolicy& zone = GetPolicyInterface<ZonePolicy>(handle);
    return zone.removeZonePolicy(name);
}
...
dpm_zone_iterator_h dpm_get_zone_iterator(dpm_zone_policy_h handle)
{
    ...
    return reinterpret_cast<dpm_zone_iterator_h>(new dpm_zone_iterator(zone.getZonePolicyList()));
}
