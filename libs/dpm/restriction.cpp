...
int dpm_restriction_get_settings_changes_state(dpm_restriction_policy_h handle)
{
	assert(handle);
...
int dpm_restriction_get_clipboard_state(dpm_restriction_policy_h handle)
{
	assert(handle);

	RestrictionPolicy& restriction = GetPolicyInterface<RestrictionPolicy>(handle);
	return restriction.getClipboardState();
}
...
int dpm_restriction_get_wifi_state(dpm_restriction_policy_h handle, int *enable)
{
    assert(handle);
    assert(enable);

    RestrictionPolicy& restriction = GetPolicyInterface<RestrictionPolicy>(handle);
    *enable = restriction.getWifiState();
...
int dpm_restriction_get_wifi_hotspot_state(dpm_restriction_policy_h handle, int *enable)
{
    assert(handle);
    assert(enable);

    RestrictionPolicy& restriction = GetPolicyInterface<RestrictionPolicy>(handle);
    *enable = restriction.getWifiHotspotState();
