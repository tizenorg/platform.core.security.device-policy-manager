...
/**
 * @addtogroup  CAPI_SECURITY_DPM_POLICY_CLIENT_MODULE
 * @{
 */
 ...
 * @see         dpm_context_create()
 */
DPM_API void dpm_context_destroy(dpm_context_h handle);
 ...
 * @see         dpm_context_add_policy_change_cb()
 */
DPM_API void dpm_context_remove_policy_change_cb(dpm_context_h handle, const char* name, int id);

/**
 * @brief       Creates the Device Policy Password handle
 * @details     This API creates device policy password handle required to
 *              the device policy password APIs.
 *              This API is also used to verify whether caller is authorized
 *              or not
 * @since_tizen 3.0
 * @param[in]   handle Device Policy Context Handle
 * @return      Device Policy Password handle on success, otherwise NULL
 * @remark      The specific error code can be obtained by using the
 *              get_last_result() method. Error codes are described in
 *              exception section.
 * @exception   #DPM_ERROR_NONE No error
 ...
 * @see         get_last_result()
 */
DPM_API dpm_password_h dpm_context_create_password_policy(dpm_context_h handle);

/**
 * @brief       Releases the Device Policy Password Handle
 * @details     This API must be called if interaction with the Device
 *              Policy Manager is no longer required.
 * @since_tizen 3.0
 ...
 */
DPM_API void dpm_context_destroy_password_policy(dpm_password_h handle);

/**
 * @brief       Creates the Device Policy Zone handle
 * @details     This API creates device zone password handle required to
 *              the device policy zone APIs.
 *              This API is also used to verify whether caller is authorized
 *              or not
 * @since_tizen 3.0
 * @param[in]   handle Device Policy Context Handle
 * @return      Device Policy Zone handle on success, otherwise NULL
 * @remark      The specific error code can be obtained by using the
 *              get_last_result() method. Error codes are described in
 *              exception section.
 * @exception   #DPM_ERROR_NONE No error
 ...
 * @see         get_last_result()
 */
DPM_API dpm_zone_h dpm_context_create_zone_policy(dpm_context_h handle);

/**
 * @brief       Releases the Device Policy Zone Handle
 * @details     This API must be called if interaction with the Device
 *              Policy Manager is no longer required.
 * @since_tizen 3.0
 * @param[in]   handle Device Policy Zone Handle
 * @return      None
 * @pre         The handle must be created by dpm_context_create_zone_policy()
 * @see         dpm_context_create_zone_policy()
 */
DPM_API void dpm_context_destroy_zone_policy(dpm_zone_h handle);
