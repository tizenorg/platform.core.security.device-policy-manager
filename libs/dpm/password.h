...
typedef struct _mem_header mem_header_t;
...
typedef struct _dpm_data dpm_data_type;
 ...
 * @since_tizen 3.0
 */
struct _dpm_password_policy {
    int quality_type;
    int min_length;
    int min_complex_char_number;
    int max_attempts;
    int valid_period;
    int history_size;
    int max_inactivity_time;
    int max_char_occurrences;
    int max_num_seqLength;
    char *pattern;
    void *forbidden_string_list;
};

typedef struct _dpm_password_policy dpm_password_policy_type;
 ...
 *    dpm_password_policy_type* passwd_policy;
 *    dpm_data = dpm_get_password_policy(handle, "owner");
 *    if (dpm_data != NULL) {
 *        passwd_policy = (dpm_password_policy_type*)dpm_data->data;
