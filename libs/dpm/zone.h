 ...
 * @retval      #DPM_ERROR_INVALID Invalid parameter
 * @retval      #DPM_ERROR_PERMISSION_DENIED The application does not have
 *              the privilege to call this API or the call is not the owner
 *              of the zone
 * @pre         handle must be created by dpm_create_client()
 ...
 * @param[in]   state The current state of the container
 ...
 */
typedef void(*dpm_zone_signal_cb)(dpm_client_h handle, const char* name, dpm_zone_state_e state, void *user_data);
 ...
 * @see         dpm_remove_zone_signal_handler()
 */
DPM_API int dpm_add_zone_signal_handler(dpm_client_h handle, dpm_zone_signal_cb handler, void* user_data);
