 ...
 *              follow this restriction either.
 * @since_tizen 3.0
 * @privlevel   public
 ...
 *              modify all Wi-fi network settings normally and also remove it.
 *              Also the apps that uses APIs that hange some Wi-Fi settings
 *              (Such as wifi_ap_set_ip_address(), wifi_ap_set_proxy_address().
 ...
 * @pre         handle must be created by dpm_create_client()
 * @post        dpm_set_wifi_network_access_restriction() must be called when SSIDs in the blacklist is needed to get restricted
 * @see         dpm_create_client()
 * @see         dpm_destroy_client()
